 ...
 * @author Ulli Hafner
 */
public class WorkspaceScanner implements FileCallable<LeaksParserResult> {
    /** Generated ID. */
    ...
    /** {@inheritDoc} */
    public LeaksParserResult invoke(final File workspace, final VirtualChannel channel) throws IOException, InterruptedException {
        String[] files = findFiles(workspace);

        LeakScanner leakScanner = new LeakScanner(high, normal, low, ignoreCase);
        LeaksParserResult result = new LeaksParserResult(files.length);
        ...
        for (String fileName : files) {
            try {
                ...
                Collection<Leak> leaks = leakScanner.scan(readFile(originalFile));
                if (!leaks.isEmpty()) {
                    ...
                    for (Leak leak : leaks) {
                        leak.setFileName(absolutePath);
                        leak.setPackageName(packageName);
                        leak.setModuleName(actualModule);
                        leak.setPathName(workspace.getPath());

                        ContextHashCode hashCode = new ContextHashCode();
                        leak.setContextHashCode(hashCode.create(absolutePath, leak.getPrimaryLineNumber(), defaultEncoding));
                    }

                    result.addAnnotations(leaks);
                }
            }
