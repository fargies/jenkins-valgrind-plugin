...
public class LeaksReporter extends HealthAwareReporter<LeaksResult> {
    ...
    /**
     * Creates a new instance of <code>LeaksReporter</code>.
     *
     * @param pattern
     *            Ant file-set pattern of files to scan for open tasks in
    ...
    public LeaksReporter(final String pattern, final String excludePattern,
    ...
    @Override
    public LeaksParserResult perform(final MavenBuildProxy build, final MavenProject pom, final MojoInfo mojo, final PluginLogger logger) throws InterruptedException, IOException {
        ...
        WorkspaceScanner workspaceScanner = new WorkspaceScanner(
                ...
                pom.getModules());
        LeaksParserResult project = basedir.act(workspaceScanner);
    ...
    protected LeaksResult createResult(final MavenBuild build, final ParserResult project) {
        return new LeaksReporterResult(build, getDefaultEncoding(), (LeaksParserResult)project, high, normal, low);
    }

    @Override
    protected MavenAggregatedReport createMavenAggregatedReport(final MavenBuild build, final LeaksResult result) {
        return new LeaksMavenResultAction(build, this, getDefaultEncoding(), high, normal, low, result);
    }

    @Override
    public List<LeaksProjectAction> getProjectActions(final MavenModule module) {
        return Collections.singletonList(new LeaksProjectAction(module, getResultActionClass()));
    }

    @Override
    protected Class<LeaksMavenResultAction> getResultActionClass() {
